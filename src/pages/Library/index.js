...
class Library extends React.Component {
  ...
  render() {
    ...
    return (
      <StyleLibrary>
        <StyleChoose>
          <Typography.Title level={3}>Defect Library</Typography.Title>
          <Form layout='vertical'>
            <Form.Item>
              <Select placeholder='Group Id' style={{ width: 120 }} onChange={id => this.selectControl(id, 'group')}>
                {groups.map(s => (
        ...
        <StyleFooter>
          ...
          <Button disabled={librarys.length <= 0} type='danger' onClick={this.onRemoveImages}>
            Remove Images
          </Button>
        </StyleFooter>
        {/* 创建Labrary */}
