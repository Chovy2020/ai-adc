export const MODAL_MODES = ['Create Library', 'Share Exist']

export const LIBRARY = [
  {
    code: 0,
    name: 'No_Review',
    characterization: 'xxxxxx',
    causeHypothesis: '111111',
    images: [
      {
        id: 1,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-25.jpg?op=OPEN',
        defect: 69
      },
      {
        id: 2,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-26.jpg?op=OPEN',
        defect: 69
      },
      {
        id: 3,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-23.jpg?op=OPEN',
        defect: 3
      },
      {
        id: 4,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-24.jpg?op=OPEN',
        defect: 3
      },
      {
        id: 14,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-1.jpg?op=OPEN',
        defect: 4
      },
      {
        id: 15,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-2.jpg?op=OPEN',
        defect: 5
      }
    ]
  },
  {
    code: 1,
    name: 'FALSE',
    characterization: 'aaaaaa',
    causeHypothesis: 'bbb',
    images: [
      {
        id: 16,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-33.jpg?op=OPEN',
        defect: 6
      },
      {
        id: 17,
        url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-34.jpg?op=OPEN',
        defect: 6
      }
    ]
  }
]

export const GALLERY_IMAGES = [
  {
    id: 1,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-25.jpg?op=OPEN',
    defect: 69
  },
  {
    id: 2,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-26.jpg?op=OPEN',
    defect: 69
  },
  {
    id: 3,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-23.jpg?op=OPEN',
    defect: 3
  },
  {
    id: 4,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-24.jpg?op=OPEN',
    defect: 3
  },
  {
    id: 14,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-1.jpg?op=OPEN',
    defect: 4
  },
  {
    id: 15,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-2.jpg?op=OPEN',
    defect: 5
  },
  {
    id: 21,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-25.jpg?op=OPEN',
    defect: 69
  },
  {
    id: 22,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-26.jpg?op=OPEN',
    defect: 69
  },
  {
    id: 23,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-23.jpg?op=OPEN',
    defect: 3
  },
  {
    id: 24,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-24.jpg?op=OPEN',
    defect: 3
  },
  {
    id: 26,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-1.jpg?op=OPEN',
    defect: 4
  },
  {
    id: 25,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-2.jpg?op=OPEN',
    defect: 5
  },
  {
    id: 31,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-25.jpg?op=OPEN',
    defect: 69
  },
  {
    id: 32,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-26.jpg?op=OPEN',
    defect: 69
  },
  {
    id: 33,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-23.jpg?op=OPEN',
    defect: 3
  },
  {
    id: 34,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-24.jpg?op=OPEN',
    defect: 3
  },
  {
    id: 35,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-1.jpg?op=OPEN',
    defect: 4
  },
  {
    id: 36,
    url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-2.jpg?op=OPEN',
    defect: 5
  }
]

export const IMAGES_LIBRARY = {
  'Group 1': {
    'ProductA_Step 1': {
      'MB 1': [
        {
          id: 1,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-25.jpg?op=OPEN',
          defect: 69
        },
        {
          id: 2,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-26.jpg?op=OPEN',
          defect: 69
        },
        {
          id: 3,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-23.jpg?op=OPEN',
          defect: 3
        },
        {
          id: 4,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-24.jpg?op=OPEN',
          defect: 3
        },
        {
          id: 14,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-1.jpg?op=OPEN',
          defect: 4
        },
        {
          id: 15,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-2.jpg?op=OPEN',
          defect: 5
        },
        {
          id: 21,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-25.jpg?op=OPEN',
          defect: 69
        },
        {
          id: 22,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-26.jpg?op=OPEN',
          defect: 69
        },
        {
          id: 23,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-23.jpg?op=OPEN',
          defect: 3
        },
        {
          id: 24,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-24.jpg?op=OPEN',
          defect: 3
        },
        {
          id: 26,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-1.jpg?op=OPEN',
          defect: 4
        },
        {
          id: 25,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-2.jpg?op=OPEN',
          defect: 5
        },
        {
          id: 31,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-25.jpg?op=OPEN',
          defect: 69
        },
        {
          id: 32,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-26.jpg?op=OPEN',
          defect: 69
        },
        {
          id: 33,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-23.jpg?op=OPEN',
          defect: 3
        },
        {
          id: 34,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-24.jpg?op=OPEN',
          defect: 3
        },
        {
          id: 35,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-1.jpg?op=OPEN',
          defect: 4
        },
        {
          id: 36,
          url: '/webhdfs/v1/ai_yei/archive/image/F0004-000-000-0001-tif-2.jpg?op=OPEN',
          defect: 5
        }
      ],
      'MB 2': [],
      'MB 3': [],
      'MB 4': []
    },
    'ProductB_Step 1': {},
    'ProductA_Step 2': {},
    'ProductB_Step 2': {}
  },
  'Group 2': {
    'ProductA_Step 1': {
      'MB 1': [],
      'MB 2': [],
      'MB 3': [],
      'MB 4': []
    },
    'ProductB_Step 1': {},
    'ProductA_Step 2': {},
    'ProductB_Step 2': {}
  },
  'Group 3': {
    'ProductA_Step 1': {
      'MB 1': [],
      'MB 2': [],
      'MB 3': [],
      'MB 4': []
    },
    'ProductB_Step 1': {},
    'ProductA_Step 2': {},
    'ProductB_Step 2': {}
  }
}
